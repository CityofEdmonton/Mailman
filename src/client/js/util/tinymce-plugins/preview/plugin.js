  ...
  define(
    ...
    function (Tools, Settings, Events) {
      ...
      var injectIframeContent = function (editor, iframe, sandbox) {
        var writeToPreviewWindow = function(content) {
          if (!sandbox) {
        ...
        var isPromise = typeof ((previewHtml || {}).then === "function");

        if (isPromise) {
          previewHtml.then(text => {
            writeToPreviewWindow(text);
          }, err => {
            console.error("Error getting editor content: " + err);
            throw err;
          }); 
        } else {
          writeToPreviewWindow(previewHtml);
        }
