...
export default class MergeTemplateInputForm extends Component {
  ...
  handleTextInputConstraint(input) {
    var constraintPass = input.match(
      new RegExp(this.props.textInputConstraintRegex)
    )
  ...
  renderTextInput() {
    if (this.props.textInputTitle) {
      return (
        <FormControl
          error={
            !this.state.textInputConstraintPass &&
            Boolean(this.state.textInputValue)
          }
        >
          <Input
            name="text_input"
          ...
          {!this.state.textInputConstraintPass &&
          Boolean(this.state.textInputValue) ? (
            <Typography variant="body2" style={styles.textInputError}>
              {this.props.textInputConstraintMessage}
            </Typography>
          ) : null}
        </FormControl>
      )
  ...
  createMenuItems() {
    return this.props.menuInputValues.map(function(tab) {
      return (
    ...
    if (this.props.menuInputTitle) {
      return (
        <FormControl name="menu_input">
          {this.state.menuInputSelected ? null : (
            <InputLabel>{this.props.menuInputTitle}</InputLabel>
          )}
          <Select
  ...
  renderTip() {
    if (this.props.tip) {
      return (
        <Tooltip title={this.props.tip} style={styles.tip}>
          <HelpIcon />
        </Tooltip>
      )
    } else {
...
MergeTemplateInputForm.propTypes = {
  ...
  textInputValue: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide a textInputValue string!')
    }
  },
  textInputCallback: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide a textInputCallback function!')
    }
  },
  textInputConstraintRegex: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide an input to check!')
    }
    if (props[propName] !== undefined && typeof props[propName] !== 'string') {
      return new Error('"textInputConstraintRegex" has to be a string!')
    }
  },
  textInputConstraintCallback: function(props, propName, componentName) {
    if (
      props['textInputConstraintRegex'] &&
      (props[propName] === undefined || typeof props[propName] !== 'function')
    ) {
      return new Error('Please provide a textInputConstraintCallback function!')
    }
  },
  ...
  formInputValue: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide a formInputValue boolean!')
    }
  },
  formInputCallback: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide a formInputCallback function!')
    }
  },
  menuInputTitle: PropTypes.string,
  menuInputSelected: function(props, propName, componentName) {
    if (
      props['menuInputTitle'] &&
      (props[propName] === undefined || typeof props[propName] !== 'string')
    ) {
      return new Error('Please provide a menuInputSelected string!')
    }
  },
  menuInputValues: function(props, propName, componentName) {
    if (props['menuInputTitle']) {
      if (props[propName] === undefined) {
        return new Error('Please provide a menuInputValues array of strings!')
      }
      for (let i = 0; i < props[propName].length; i++) {
        if (typeof props[propName][i] !== 'string') {
          return new Error('menuInputValues is not an array of strings!')
        }
      }
    }
  },
  menuInputCallback: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please procide a menuInputCallback function!')
    }
  },
}
