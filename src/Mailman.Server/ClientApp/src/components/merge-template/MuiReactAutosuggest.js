...
function getSuggestions(value, inputProps) {
  ...
  const regex = openWrapper
    ? new RegExp(openWrapper + '[^' + closeWrapper + ']*$')
    : new RegExp('')
  ...
  const inputValue = openWrapper
    ? value
        .slice(sliceFrom, value.length)
        .trim()
        .toLowerCase()
...
class MuiReactAutosuggest extends React.Component {
  ...
  handleConstraint = input => {
    var newConstraintPass = Boolean(
      input.match(new RegExp(this.props.constraintRegex))
    )
  ...
  onSuggestionSelected = (event, { suggestion }) => {
    ...
    const newValue = regex
      ? this.state.single.replace(
          regex,
          openWrapper + suggestion.label + closeWrapper
        )
      : suggestion.label
  ...
  renderSuggestion = (suggestion, { query, isHighlighted }) => {
    ...
    const newQuery = regex
      ? query
          .slice(sliceFrom, query.length)
          .trim()
          .toLowerCase()
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        />
        {!this.state.constraintPass && Boolean(this.state.single) ? (
          <Typography variant="body2" style={{ marginTop: 5, color: 'red' }}>
            {this.props.constraintMessage}
          </Typography>
        ) : null}
      </FormControl>
    )
...
MuiReactAutosuggest.propTypes = {
  ...
  closeWrapper: function(props, propName, componentName) {
    if (
      ...
      return new Error('Please provide a closeWrapper function!')
    }
  },
  constraintRegex: function(props, propName, componentName) {
    if (props[propName] !== undefined && props['callback'] === undefined) {
      return new Error('Please provide a callback function!')
    }
    if (props[propName] !== undefined && typeof props[propName] !== 'string') {
      return new Error('"constraintRegex" has to be a string!')
    }
  },
  constraintCallback: function(props, propName, componentName) {
    if (
      props['constraintRegex'] &&
      (props[propName] === undefined || typeof props[propName] !== 'function')
    ) {
      return new Error('Please provide a constraintCallback function!')
    }
  },
